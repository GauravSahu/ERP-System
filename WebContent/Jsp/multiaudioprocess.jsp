    ...
    System.out.println("company id in jsp "+compid);
				...
				//	}
	    	    	...
	    	    	try{
	    	    		...
	    	    		if (item.getSize() > 0){
							...
							uploadedFile = new File(getServletContext().getRealPath("\\")+filetype,uid+myFileName);
  	...
  	//if(invalidFile.equalsIgnoreCase("")){
          ...
          ResultSet rs = null;
	          ...
	          String query = "select emailid from usermaster where userid=?";
	          preStat = con.prepareStatement(query);
	          preStat.setLong(1, uid);
	    	...
	    	for(int i=0;i<counterNoOfFiles;i++){
	    		//System.out.println("175"+allfileName.size());
	   	   		...
	   	   		try{
		    	    	...
		    	    	String query = "insert into filemanager (filename,userid,compid,filetype) values(?,?,?,?)";
		    	    	psmt=con.prepareStatement(query);
		    	    	psmt.setString(1, uid+allfileName[i].toString());
		    	    	psmt.setInt(2,uid);
		    			psmt.setLong(3,compid);
...
<br/>
 <table align="center"> 
   <form action="<%=request.getContextPath() %>/mail.do?action=sendmail" method="post">
   <center><tr><td><input type="hidden" name="mailid" value=<%=id%>></input></td>
   <td> <input type="submit" value="Send mail to member"></input></td></tr></center>
   </form>
 </table> 
...
<div id="social">
<table width="216" style="margin-top:6px;">
<tr>
<td width="89" height="44" class="follow"> Follow us on :</td>
<td width="32"><a href="https://www.facebook.com"><img src="<%=request.getContextPath() %>/images/facebook.png" border="0" /></a></td>
<td width="32"><a href="https://www.twitter.com"><img src="<%=request.getContextPath() %>/images/twitter.png" border="0" /></a></td>
<td width="43"><a href="https://www.youtube.com"><img src="<%=request.getContextPath() %>/images/youtube.png" border="0" /></a></td>
</tr>
</table>
